...
public class TrainingEnrollmentService {
    ...
    public boolean isAlreadyEnrolled(Long employeeId, Long courseId) {
        logger.info("Checking enrollment status for employee: {} and course: {}", employeeId, courseId);
        return trainingEnrollmentRepo.existsByEmployeeEmployeeIdAndCourseCourseId(employeeId, courseId);
    }
    ...
    public TrainingEnrollmentDTO convertToDTO(TrainingEnrollment entity) {
        ...
        if (entity.getEmployee() != null && entity.getEmployee().getDepartment() != null) {
            Department dept = entity.getEmployee().getDepartment();
            deptName = dept.getDepartmentName();
            availBalance = dept.getAvailableBalance();
        }

        return TrainingEnrollmentDTO.builder().enrollmentId(entity.getEnrollmentId()).enrollmentDate(entity.getEnrollmentDate()).remarks(entity.getRemarks()).requestedCost(entity.getRequestedCost()).approvedCost(entity.getApprovedCost()).currentApprovalLevel(entity.getCurrentApprovalLevel()).completionDate(entity.getCompletionDate()).certificateIssued(entity.getCertificateIssued()).createdAt(entity.getCreatedAt()).employeeId(entity.getEmployee() != null ? entity.getEmployee().getEmployeeId() : null).employeeFullName(entity.getEmployee() != null ? entity.getEmployee().getFirstName() + " " + entity.getEmployee().getLastName() : "N/A").departmentName(deptName != null ? deptName : "N/A").availableBalance(availBalance).courseId(entity.getCourse() != null ? entity.getCourse().getCourseId() : null).courseName(entity.getCourse() != null ? entity.getCourse().getCourseName() : "N/A").enrollmentStatusId(entity.getEnrollmentStatus() != null ? entity.getEnrollmentStatus().getEnrollmentStatusId() : null).enrollmentStatusName(entity.getEnrollmentStatus() != null ? entity.getEnrollmentStatus().getEnrollmentStatus() : "PENDING").build();
    }
    ...
    public List<TrainingEnrollmentDTO> getCertifiedEnrollmentsByEmployee(Long employeeId) {
        return trainingEnrollmentRepo.findByEmployeeEmployeeId(employeeId).stream().filter(enrollment -> enrollment.getEnrollmentStatus() != null && "Certified".equals(enrollment.getEnrollmentStatus().getEnrollmentStatus())).map(this::convertToDTO).collect(Collectors.toList());
    }
    ...
    public void approveEnrollment(Long enrollmentId, Long approverId, BigDecimal approvedCost, String comments) {
        ...
        if (enrollment.getEnrollmentStatus() != null && "Approved".equals(enrollment.getEnrollmentStatus().getEnrollmentStatus())) {
            logger.warn("Enrollment record {} is already fully approved. Preventing duplicate adjustments.", enrollmentId);
            return;
        }
        ...
        if (!hasMoreLevels) {
            ...
            if (employee == null || employee.getDepartment() == null) {
                throw new IllegalStateException("Cannot finalize approval. Employee or Department context missing.");
            }

            Department department = employee.getDepartment();
            BigDecimal availableBalance = department.getAvailableBalance() != null ? department.getAvailableBalance() : BigDecimal.ZERO;

            if (availableBalance.compareTo(approvedCost) < 0) {
                String errorMessage = String.format("Insufficient department balance. Required: ₹%s, Available: ₹%s in department: %s", approvedCost, availableBalance, department.getDepartmentName());
                logger.error("Approval Rejected at final tier: {} for Enrollment ID: {}", errorMessage, enrollmentId);
                throw new RuntimeException(errorMessage);
            }

            BigDecimal dynamicNewBalance = availableBalance.subtract(approvedCost);
            department.setAvailableBalance(dynamicNewBalance);
            departmentRepo.save(department);

            String balanceDetails = String.format("Deducted ₹%s from Department: %s. Previous Balance: ₹%s, New Balance: ₹%s", approvedCost, department.getDepartmentName(), availableBalance, dynamicNewBalance);
            logger.info("Budget balance successfully locked: {}", balanceDetails);
            auditLogService.logAudit(department.getDepartmentId(), "UPDATE", "departments", balanceDetails);
        ...
        auditLogService.logAudit(trainingApproval.getApprovalId(), "INSERT", "training_approvals", "Enrollment approved by approver id: " + approverId + ", Comments: " + comments);
    ...
    public List<TrainingEnrollmentDTO> getApprovedEnrollmentDTOs() {
        return trainingEnrollmentRepo.findAll().stream().filter(enrollment -> enrollment.getEnrollmentStatus() != null && !"Pending approval".equalsIgnoreCase(enrollment.getEnrollmentStatus().getEnrollmentStatus())).map(this::convertToDTO).collect(Collectors.toList());
    }
}